...
def parse_tree(projects, allowed_task_time):
    ...
    for goal in projects:
        ...
        try:
            parsedDeadline = \
            re.search(deadlineRegex, goal["nm"], re.IGNORECASE)[1]
            goalDeadline = int((datetime.strptime(parsedDeadline,
                                                  "%Y-%m-%d").date() - current_date).days)
        except:
            goalDeadline = None
            missing_deadlines += 1
        ...
        for child_idx, child in enumerate(goal["ch"]):
            ...
            if duration > allowed_task_time:
                raise cherrypy.HTTPError(403, f"Task \"{child['nm']}\" has duration more than the allowed time duration!")
...
def tree_to_old_structure(projects, day_duration):
    ...
    old_structure = []
    for goal in projects:
        if goal["code"] in [str(goal_num) for goal_num in
                            range(11)]:  # don't include miscellaneous
            old_structure.append(
                Goal(goal["id"], [Task(task["id"], task["est"])
                                  for task in goal["ch"]],
