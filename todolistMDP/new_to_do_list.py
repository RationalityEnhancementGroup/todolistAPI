...
class MDP:
    ...
    def compute_start_state_pseudo_rewards(self, bias=None, scale=None, flag=False):
        ...
        for item, q in self.Q_s0.items():
            ...
            print(f'Item: {item.description} future_q: {future_q}, q - reward: {q - reward}')
        ...
        sum_item_values = sum([self.value + item.intrinsic_reward for item in items])
...
class MainToDoList:
    ...
    def solve(self, start_time=None, verbose=False):
        ...
        for node in self.tree:
            ...
            self.solve_MDP(self.nodes[node], self.tree[node], start_time, verbose, flag=False)

        prs = self.scale_bias()
        print(f'*********** Scaled ***********')
        # Convert task queue to a list
        optimal_tasks = list(self.tasks)
        ...
        for task in optimal_tasks:
            print(f'Task: {task.description}, Op: {task.get_optimal_reward()},  PRS: {prs["id2pr"][task.description]}')
        return
    ...
    def scale_bias(self, bias=None, scale=None):
        ...
        sum_pr = sum([self.pr_dict[task.description] for task in self.tasks])
        sum_item_values = sum([self.value_dict[task.description] for task in self.tasks])
        ...
        for item in self.tasks:
            ...
            # Transform pseudo-reward
            pr = f = scale * self.pr_dict[item.description] + bias
            # Get expected item reward
            item_reward = self.expected_dict[item.description]

            print(f'{item.description}: r: {item_reward}, pr: {pr}, new_pr: {pr + item_reward}')
            print(f'Scale: {scale} Optimal Reward: {self.pr_dict[item.description]}, Bias: {bias}')
            # Add immediate reward to the pseudo-reward
            pr += item_reward
